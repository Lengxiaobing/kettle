...
import org.kettle.sxdata.service.impl.JobServiceImpl;
    ...
    private static class JobTimerTask extends Task {
        ...
        public void run() {
            try {
                ...
                job = newJob(org.kettle.sxdata.util.quartz.JobTimerTask.class).withIdentity(idJobTask + "", JOB_TIMER_TASK_GROUP).build();
...
//                    }
    ...
    public static void startJobTimeTask(DefaultSqlSessionFactory bean) throws Exception {
        ...
        List<JobTimeSchedulerEntity> jobsTimer = session.selectList("org.kettle.sxdata.dao.JobSchedulerDao.getAllTimerJob", "");
        if (jobsTimer == null || jobsTimer.size() < 1) {
            ...
            for (JobTimeSchedulerEntity timerJob : jobsTimer) {
                ...
                //根据id获取当前作业
                JobEntity thisJob = session.selectOne("org.kettle.sxdata.dao.JobDao.getJobById", jobId);
