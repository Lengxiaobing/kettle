...
public class JobServiceImpl implements JobService {
    ...
    public void deleteJobs(String jobPath, String flag) throws Exception {
        ...
        if (timers != null && timers.size() > 0) {
            ...
            for (JobTimeSchedulerEntity timer : timers) {
                ...
                //删除作业
                scheduler.deleteJob(JobKey.jobKey(taskId, CarteTaskManager.JOB_TIMER_TASK_GROUP));
            }
        }
        //删除作业在任务组明细表中的记录
    ...
    public boolean judgeJobIsAlike(JobTimeSchedulerEntity willAddJobTimer) {
        ...
        if (jobs != null && jobs.size() >= 1) {
            //存放作业名与用户选择的相同的作业
            List<JobTimeSchedulerEntity> jobNameEqual = new ArrayList<>();
            ...
            if (jobNameEqual.size() >= 1) {
                //存放定时类型与用户传递的定时运行相同的作业
                List<JobTimeSchedulerEntity> typeEqualJobs = new ArrayList<>();
                ...
                if (typeEqualJobs.size() >= 1) {
                    ...
                    switch (timerType) {
                        ...
                        case 2:
                            //存放每天执行 小时与用户设置值一样的定时作业
                            List<JobTimeSchedulerEntity> hourLikeByTypeTwo = new ArrayList<>();
                            for (JobTimeSchedulerEntity jobTime : typeEqualJobs) {
                        ...
                        case 3:
                            ...
                            //存放星期数值与用户传递值相同的记录
                            List<JobTimeSchedulerEntity> weekLikeByTypeThree = new ArrayList<>();
                            for (JobTimeSchedulerEntity jobTime : typeEqualJobs) {
                                if (jobTime.getWeekday() == willAddJobTimer.getWeekday()) {
                                    weekLikeByTypeThree.add(jobTime);
                                }
                            }
                            //存放星期数值和小时与用户传递值相同的记录
                            List<JobTimeSchedulerEntity> weekAndHourLikeByTypeThree = new ArrayList<>();
                            //如果星期值相同判断小时是是否也相同
    ...
    public boolean beforeTimeExecuteJob(Map<String, Object> params, HttpServletRequest request) throws Exception {
        ...
        //定时的ID 唯一标识
        jobTimeScheduler.setIdJobtask(System.nanoTime());
        //所需定时的作业ID
        jobTimeScheduler.setIdJob(Integer.parseInt(params.get("jobId").toString()));
        //作业名
        jobTimeScheduler.setJobName(params.get("jobName").toString());
        //是否重复执行 暂时默认重复执行Y
        jobTimeScheduler.setIsrepeat("Y");
        //设置定时类型
